function [barPos,image,gof]  = poletracker(fname, radius,frames,drawing)

% helper function for extracting pole location
% 
% Originally created RSP 040914
% Developed by Mathew Evans since 14.10.2014
%
% AIM: use this script to develop pole tracking code and to find
% parameters, eg for the pole radius

% Updated 25.09.15 to accept both .avi and .dat files (though .avi may
% hang)
...
handles.pole.roi = [1 300 1 300]; %[1 350 1 60];   % x,ys
...
switch handles.fname(end-2:end)
    ...
    case 'dat'
        ...
        video.start_frame=video.header.startframe;
        video.offset = 8192;
...
for frameidx = frames; %video.header.nframes
    ...
    % image
    % gof = goodness of fit
    [polecentre(i,:), gof(i)] = polefit(handles.frame,handles.pole.radius,handles.pole.roi);
