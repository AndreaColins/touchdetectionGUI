...
cd('B:\Dario\Behavioral_movies\34\130315_34a\2015_03_13')
%clear all;
...
plotting = 0; % set to 1 if you want to plot as you go, good for debugging

%% Load measurements and whiskers files for a given loadfile
% Uncomment next 6 lines  to run in 'function' mode
% clear all
% files = dir('*.avi');
for fidx = 37:numel(files); %
    plotting = 1;
    tic
    loadfile = files(fidx).name(1:end-4); % was -4 for .dat or .avi, -3 for .tr
% loadfile = [session,'_',vid,'_',file];

disp(['CHIMERA: pre-processing trial ',loadfile])
...
save([loadfile,'_clean.mat'],'whisker_backbone','best_m','best_w');
...
roi.x = 1:400; roi.y = 100:270; % Needs adjusting from session to session
...
i=0; % for easier debugging below
%% execution options
plotting = 0; % plot as you go
...
tic
% clf
for i = i+1: numel(x); % 1570; %1500
    i
    
    % NOW FEATURING A TRY-CATCH LOOP
    try
        ...
        % Quadratic fit to face - now linear
        ...
        disp(['CHIMERA ERROR: skipping frame ',num2str(i)])
...
clear ax i image s_contour mum1 pm1 r rall temp wp dr
toc
% Uncomment end to allow running in batch mode
end
